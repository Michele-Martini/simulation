...
class MOTOR:
    def __init__(self, jointName):
        ...
        #self.Prepare_To_Act(1)

    def Prepare_To_Act(self, scaling_factor):
        self.amplitude = c.amplitude_b
        self.frequency = c.frequency_b * scaling_factor
        self.offset = c.phaseOffset_b
        for t in range(c.n):
            self.motorValues[t] = self.amplitude * np.sin(self.frequency * t + self.offset)
        print('Frequency of {} = {}'.format(self.jointName, self.frequency))
